...
function ImageGallery({images, cardSelected, toggleCardSelected}) {
    ...
    const renderSmallImages = images.map( i =>{
        return <button key={i.id} onClick={()=>setSelectedImage(i.image)}> <img loading='lazy' className={`h-[30px] sm:h-[50px] border-white border-2 hover:animate-scale-large`} alt={i.id} src={i.image} /></button>
    })
